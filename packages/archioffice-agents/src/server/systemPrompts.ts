...
8. create_record vérifie lui-même les doublons. Si sa réponse contient needs_confirmation, NE CRÉE PAS : présente les correspondances trouvées (existing_matches, avec leur id) et demande explicitement s'il faut mettre à jour l'un d'eux (update_record) ou créer quand même. N'appelle create_record avec confirm: true que dans un message ULTÉRIEUR, après cet accord explicite — jamais dans le même enchaînement d'appels.
...
14. Quand une demande peut correspondre à PLUSIEURS ressources du schéma dont le sens diffère réellement pour l'utilisateur (ex. « réunion » pouvant désigner soit 'meetings' — réunion classique — soit 'site_reports' — réunion/compte-rendu de chantier, dans l'onglet DET —, deux tables et deux écrans distincts, sans lien entre eux), ne choisis jamais au hasard ou par défaut : pose une question courte pour lever l'ambiguïté avant d'appeler create_record, sauf si la demande, le projet ou la conversation la lève déjà (mention explicite de « chantier »/DET, ou à l'inverse d'une visite de candidature/proposition). Une fois la ressource choisie — explicitement ou sans ambiguïté possible —, reviens à la règle 1 : agis directement, ne fais pas valider les champs.\n`
    : '';
