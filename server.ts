...
- "prescriptionsTechniques": normes et prescriptions techniques applicables (1-2 phrases)`;
  ...
  function buildAgentSystemPrompt(agent: AgentRow, ctx: AgentContext): string {
...
4. N'invente jamais de données (noms, dates, montants, références).`;
  ...
  async function buildAgentContext(tenantId: string, userId: string, scopes: string[]): Promise<AgentContext> {
    ...
    const ctx: AgentContext = {
      tenantName: (tenantRes.data as any)?.name ?? 'Cabinet',
      currentDate: new Date().toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
      currentUserName: (profileRes.data as any)?.name ?? 'Utilisateur',
      projects: [],
      contacts: [],
      upcomingMeetings: [],
      recentDocuments: [],
    ...
    if (scopes.includes('projects')) {
      fetches.push(supabaseAdmin.from('projects').select('id, name, status, client, start_date, end_date').eq('tenant_id', tenantId).neq('status', 'Completed').order('created_at', { ascending: false }).limit(30).then((r: any) => { ctx.projects = r.data || []; }));
    }
    if (scopes.includes('contacts')) {
      fetches.push(supabaseAdmin.from('contacts').select('id, first_name, last_name, company_name, email').eq('tenant_id', tenantId).limit(50).then((r: any) => { ctx.contacts = r.data || []; }));
    }
    if (scopes.includes('meetings')) {
      fetches.push(supabaseAdmin.from('meetings').select('id, title, date, project_id').eq('tenant_id', tenantId).gte('date', new Date().toISOString()).order('date', { ascending: true }).limit(10).then((r: any) => { ctx.upcomingMeetings = r.data || []; }));
    }
    if (scopes.includes('documents')) {
      fetches.push(supabaseAdmin.from('documents').select('id, name, project_id, phase, uploaded_at').eq('tenant_id', tenantId).order('uploaded_at', { ascending: false }).limit(15).then((r: any) => { ctx.recentDocuments = r.data || []; }));
    }
    if (scopes.includes('tasks')) {
      fetches.push(supabaseAdmin.from('tasks').select('id, title, status, due_date, project_id').eq('tenant_id', tenantId).neq('status', 'done').order('due_date', { ascending: true }).limit(20).then((r: any) => { ctx.tasks = r.data || []; }));
    }
  ...
  // GET /api/agent-templates — liste les templates système
  ...
  // GET /api/agents — agents actifs du tenant
  ...
  // POST /api/agents — activer un template ou créer un agent custom
  ...
  // PUT /api/agents/:id — modifier un agent du tenant
  ...
  // DELETE /api/agents/:id — désactive un agent
  app.delete("/api/agents/:id", async (req: any, res: any) => {
    try {
      const tenantId = await getTenantId(req.user.id);
      const { id } = req.params;
      const { data: agent } = await supabaseAdmin.from('agents').select('is_system_template').eq('id', id).eq('tenant_id', tenantId).single();
  ...
  // GET /api/agents/:id/conversation — récupère ou crée la conversation + historique
  ...
  // DELETE /api/agents/:id/conversation — réinitialise la conversation
  app.delete("/api/agents/:id/conversation", async (req: any, res: any) => {
    try {
      ...
      const { data: conv } = await supabaseAdmin.from('agent_conversations').select('id').eq('agent_id', agentId).eq('user_id', userId).eq('tenant_id', tenantId).single();
      if (conv) {
  ...
  // GET /api/agents/token-balance — solde tokens du tenant
  ...
  // POST /api/agents/:id/chat — envoie un message à un agent
  app.post("/api/agents/:id/chat", async (req: any, res: any) => {
    try {
      ...
      // Contexte
      const ctx = await buildAgentContext(tenantId, req.user.id, (agent as any).context_scopes || []);
      ...
      const newBalance = Math.max(0, balance - tokensUsed);
      res.json({ reply, tokens_used: tokensUsed, remaining_balance: newBalance });
    } catch (e: any) {
