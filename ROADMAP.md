...
- **No pagination on most list endpoints** (`/api/documents`, `/api/tasks`, `/api/tenders`, `/api/rfis`, `/api/reserves`, `/api/meetings`, and others) — large tenants get full, unpaged arrays back. `/api/projects` and `/api/invoices` got opt-in cursor pagination (`?limit=&cursor=`, returning `{ data, nextCursor }`) and dropped their per-row relational fan-out (cotraitants/lots/stakeholders/categories, line items) down to a single per-item detail fetch — see CLAUDE.md's "Pagination et fan-out sur les listes" — but the remaining endpoints above still return everything, unpaged, on every call.
- **Same gap, but silently data-lossy: a plain `.select('*')` with no `.range()`/`.limit()` gets capped at 1000 rows by Supabase/PostgREST (`db-max-rows`) with no error.** Past that count for a given tenant, the response is quietly truncated — and without an `ORDER BY`, nothing guarantees the kept rows are the oldest ones, so a recently created row can simply be missing from the list with no error or filter to explain it. Hit in production on `GET /api/contacts` (AAZS tenant, 1621 contacts — a same-day contact was outside the 1000 rows returned) and fixed by paging through all rows server-side (`fetchAllContacts()` in `server/routes/contacts.ts`, same loop-of-`.range()` pattern as `fetchAllRows()` in `server/tenantExport.ts`) rather than by exposing pagination to the client. Every other unpaginated list above shares the same exposure once a tenant crosses 1000 rows on that table — `/api/documents` and `/api/tasks` are the most likely next ones to hit it given typical row counts — and should get the same treatment (page through server-side, keep returning the full array) rather than waiting for another silent-data-loss report.
