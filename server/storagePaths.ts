...
// "feed-attachments" storage buckets, which store business documents and are
// now private (see ensureStorageBuckets in server.ts and the security audit —
// they used to be `public: true`, so any URL leak exposed the file with no
// auth, no tenant check, and no expiry). `uploadToStorage()` in server.ts
// still builds/returns a "public URL"-shaped string (getPublicUrl() just
// constructs that string; it doesn't care whether the bucket is actually
// public), so every existing DB row keeps working as a stable reference —
// it's just no longer directly fetchable. server/routes/storageAccess.ts
// exchanges that reference for a short-lived signed URL after checking the
// caller's tenant owns it.
