...
export function registerTenderRoutes(app: Express, { supabaseAdmin, getTenantId, getUserName, logActivity, captureWithContext }: RouteDeps) {
  ...
  // (entreprise retenue) est connu, retrouvées par type de procédure ou par
  ...
  // (specialties jointes séparément) plutôt que via un embed PostgREST, pour
  // rester simple sur un nombre d'affaires par cabinet toujours modeste.
  app.get("/api/tenders/:id/candidatures-similaires", async (req: any, res: any) => {
    try {
      ...
      const { data: candidates } = await tenantScopedFrom(supabaseAdmin, tenantId, 'tenders')
        .select('id, title, client, type, entreprise_retenue, honoraires_retenus_montant, enveloppe_previsionnelle, submission_deadline')
        .not('entreprise_retenue', 'is', null)
        .neq('id', id);
      ...
      const similar = candidateList
        ...
        .slice(0, 10)
        .map(c => ({ id: c.id, title: c.title, client: c.client, type: c.type, entreprise_retenue: c.entreprise_retenue, honoraires_retenus_montant: c.honoraires_retenus_montant, enveloppe_previsionnelle: c.enveloppe_previsionnelle, submission_deadline: c.submission_deadline }));
